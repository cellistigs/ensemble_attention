...
class Test_AttnComparison():
    ...
    def test_forward_rot(self):    
        ...
        assert np.allclose(overlap.detach().numpy(),softmax(np.array([[0,0,0,1],[0,1,0,0],[1,0,1,0]])/np.sqrt(3),axis = 1))
        assert overlap.shape == (1,3,4)


class Test_AttnMLPComparison():
    """We want to test that this attention head works as intended. 
    ...
    def test_init(self):
        ...
        out_dim = 100
        scale = np.sqrt(out_dim) ## sqrt of 100
        layer = layers.AttnMLPComparison(in_dim,out_dim)
        assert layer.mlp_input_q.weight.shape == (1000,3)
        assert layer.mlp_input_k.weight.shape == (1000,3)
        assert layer.mlp_output_q.weight.shape == (100,1000)
        assert layer.mlp_output_k.weight.shape == (100,1000)
        assert layer.scale == 1/scale

    def test_forward(self):    
        """Now we want to ensure that compute works as intended. Just make sure we get all the way through.  
        """
        ## initialize test input:
        np_queries = np.array([[0,0,1],[0,1,0],[1,0,0]]).reshape(1,3,3)
        np_keys = np.array([[1,0,0],[0,1,0],[1,0,0],[0,0,1]]).reshape(1,4,3)
        ...
        ## initialize weights: 
        layer = layers.AttnMLPComparison(3,3).double()
