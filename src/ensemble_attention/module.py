...
class CIFAR10EnsembleDKLModule(CIFAR10EnsembleModule):
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.SGD(
            self.models.parameters(),
            #lr=self.hparams.learning_rate, ## when jointly training, we need to multiply the learning rate times the number of ensembles to make sure that the effective learning rate for each model stays the same. 
            lr=self.hparams.learning_rate*len(self.models)/2, ## when jointly training, we need to multiply the learning rate times the number of ensembles to make sure that the effective learning rate for each model stays the same. 
            weight_decay=self.hparams.weight_decay,
            momentum=0.9,
            nesterov=True,
        )
        ...
        if self.hparams.scheduler in [None,"cosine"]: 
            scheduler = {
                "scheduler": WarmupCosineLR(
                    ...
                    warmup_epochs=total_steps*0.3,
                    max_epochs=total_steps,
                    warmup_start_lr = 1e-8*len(self.models)/2,
                    eta_min = 1e-8*len(self.models)/2
                    ),
                "interval": "step",
                "name": "learning_rate",
            }
