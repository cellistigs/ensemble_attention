...
results_dir = os.path.join(base_dir,"/data/Projects/linear_ensembles/ensemble_attention/etc/train_gamma_ensembles/multirun/2022-12-13/12-29-27/")
acc = AccuracyData()
...
def plot(all_data_ind,all_data_ood):    
    ...
    plt.suptitle(r'ResNet 18 Ensemble (M = 4) Weighted Training: $L_{test}+ \gamma D_{KL}(p_{unif}\|p_i= \frac{f_i}{(\sum_if_i)})$')
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir,"kl_weight_resnet18_cifar10.pdf"))
