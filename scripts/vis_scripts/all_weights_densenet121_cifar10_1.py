...
interpensembles_results = os.path.join(here,"../../../interp_ensembles/results")
acc = AccuracyData()


## For visualization scripts, we do not take parameters. 
base_resnets = {
    "ResNet 8":os.path.join(interpensembles_results,"robust_results11-17-21_00:55.03_base_densenet121"),#"resnet8_0/",
    "ResNet 8.7":os.path.join(interpensembles_results,"robust_results11-17-21_01:36.53_base_densenet121"),
    "ResNet 8.8":os.path.join(interpensembles_results,"robust_results11-17-21_02:19.01_base_densenet121"),
    "ResNet 8.9":os.path.join(interpensembles_results,"robust_results11-17-21_03:01.03_base_densenet121"),
...
datafolder_dkl = "test_kl_data_densenet121_cifar10_1"
datafolder_p2b = "test_p2b_data_densenet121_cifar10_1"
datafolder_js_avg = "test_js_avg_data_densent121_cifar10_1"
datafolder_js_unif = "test_js_unif_data_densenet121_cifar10_1"
...
def plot_main(all_data_ind,all_data_ood):    
    ...
    for i,reg in enumerate(["dkl","p2b","js_avg","js_unif"]):
        ...
        #ax[i].axhline(y = all_data_ood["baselines"]["single_models"]["mean"],color = "black",label = "single model")
        #
        ensemble_upper = all_data_ood["baselines"]["ensembles_joint"]["mean"]+all_data_ood["baselines"]["ensembles_joint"]["se"]
        ensemble_lower = all_data_ood["baselines"]["ensembles_joint"]["mean"]-all_data_ood["baselines"]["ensembles_joint"]["se"]
        ax[i].fill_between(x,ensemble_upper,ensemble_lower,color = "blue",alpha = 0.5)
        ax[i].axhline(y = all_data_ood["baselines"]["ensembles_joint"]["mean"],color = "blue",label = "ensemble ($\gamma = 0$)")


        ax[i].axvline(x = 0,linestyle = "--",color = "black")
        for gamma, model in all_data_ood["gamma_weighted"][reg]["gammas"].items():
    ...
    plt.suptitle(r'DenseNet 121 Ensemble (M = 4)' +"\n"+'Weighted Training: CIFAR10.1',size = "xx-large")
    ax[0].legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir,"all_weight_densenet121_cifar10_1.pdf"))
