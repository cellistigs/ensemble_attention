...
from ensemble_attention.data import ImagenetData
from ensemble_attention.module_imagenet import TinyImagenetModule, TinyImagenetEnsembleModule, \
    ImagenetModule, ImagenetEnsembleModule, ImagenetEnsembleDKLModule

modules = {
        #"base":CIFAR10Module,
        #"base_tinyimagenet":TinyImagenetModule,
        "base_imagenet":ImagenetModule,
        #"ensemble":CIFAR10EnsembleModule,  # train time ensemble
        #"ensemble_tinyimagenet":TinyImagenetEnsembleModule,  # train time ensemble
        "ensemble_imagenet": ImagenetEnsembleModule,  # train time ensemble
        "ensemble_dkl_imagenet": ImagenetEnsembleDKLModule,  # train time ensemble
...
@hydra.main(config_path = os.path.join(script_dir,"../configs/"),config_name = "run_default_cpu_imagenet")
def main(args):
...
#        os.environ["CUDA_VISIBLE_DEVICES"] = args.gpu_id
    ...
    project = args.get("project", "imagenet")
        ...
        if bool(args.pretrained):
            if args.pretrained_path is None:
                state_dict = os.path.join(
                    script_dir,"../","models",
                    "imagenet_models", "state_dicts", args.classifier + ".pt"
                )
            else:     
